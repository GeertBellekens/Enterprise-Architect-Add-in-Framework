...
{
	/// <summary>
	/// Description of MySQLTableTransformer.
	/// </summary>
	public class MySQLTableTransformer:EATableTransformer
	{
		protected List<MySQLColumnTransformer> _columnTransformers = new List<MySQLColumnTransformer>();
		internal Dictionary<UTF_EA.AssociationEnd,MySQLTableTransformer> dependingTransformers = new Dictionary<UTF_EA.AssociationEnd, MySQLTableTransformer>();
		internal List<MySQLForeignKeyTransformer> _foreignKeyTransformers = new List<MySQLForeignKeyTransformer>();
		internal MySQLPrimaryKeyTransformer _primaryKeyTransformer = null;
		/// <summary>
		/// constructor
		/// </summary>
		/// <param name="database">the database the table should belong to</param>
		/// <param name="nameTranslator">the nametranslator</param>
		public MySQLTableTransformer(Database database,NameTranslator nameTranslator):base(database,nameTranslator){}
		...
		protected override void createTable(UTF_EA.Class classElement)
		{
			...
			this.table = new Table(_database, classElement.alias==""? classElement.name: classElement.alias);
			 
		}
		
		protected override Column transformLogicalAttribute(UTF_EA.Attribute attribute)
		{
			var columnTransformer = new MySQLColumnTransformer(this._table,this._nameTranslator);
		...
		public override List<DB.Transformation.ColumnTransformer> columnTransformers {
			...
			set { _columnTransformers = value.Cast<MySQLColumnTransformer>().ToList();}
		}
		...
		public override DB.Transformation.PrimaryKeyTransformer primaryKeyTransformer {
			...
			set {
				this._primaryKeyTransformer = (MySQLPrimaryKeyTransformer)value;
			}
		}
		#endregion
		#region implemented abstract members of EATableTransformer
		...
		public override List<DB.Transformation.ForeignKeyTransformer> foreignKeyTransformers {
			...
			set { _foreignKeyTransformers = value.Cast<MySQLForeignKeyTransformer>().ToList();}
		}
		...
		public void addRemoteColumnsAndKeys()
		{
			...
			foreach (var dependKeyValue in this.dependingTransformers) 
			{
				if (dependKeyValue.Value.table.primaryKey != null)
				{
					...
					{
						dependingColumnTransfomers.Add( new MySQLColumnTransformer(this._table, column,dependKeyValue.Value,dependKeyValue.Key,_nameTranslator));
					}
			...
			{
				this._primaryKeyTransformer = new MySQLPrimaryKeyTransformer(_table, PKInvolvedColumns,_nameTranslator);
			}
