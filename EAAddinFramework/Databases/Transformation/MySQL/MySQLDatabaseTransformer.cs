...
{
	/// <summary>
	/// Description of MySQLDatabaseTransformer.
	/// </summary>
	public class MySQLDatabaseTransformer:EADatabaseTransformer
	{
		private string outputName = "MySQLDatabaseTransformer.complete";
		internal List<MySQLTableTransformer> externalTableTransformers = new List<MySQLTableTransformer>();
		internal Database _externalDatabase;
		public MySQLDatabaseTransformer(Package logicalPackage,NameTranslator nameTranslator, StrategyFactory strategyFactory, bool compareOnly = false):this(logicalPackage.EAModel, nameTranslator,strategyFactory,compareOnly)
		{
			this._logicalPackage = logicalPackage;
		}
		public MySQLDatabaseTransformer(Model model,NameTranslator nameTranslator,StrategyFactory strategyFactory,bool compareOnly = false):this(getFactory(model,strategyFactory),model,nameTranslator,compareOnly)
		{
		}
		public MySQLDatabaseTransformer(DatabaseFactory factory,Model model,NameTranslator nameTranslator,bool compareOnly = false):base(factory,model,nameTranslator,compareOnly)
		{
		...
		{
			return DB_EA.DatabaseFactory.getFactory("MySQL",model, strategyFactory);
		}
		...
		{
				addMySQLTable(classElement);
		}
		protected MySQLTableTransformer addMySQLTable(AssociationEnd associationEnd)
		{
			
			MySQLTableTransformer transformer = addMySQLTable(associationEnd.type as Class);
			if (transformer == null)
			{
				transformer = this.tableTransformers.OfType<MySQLTableTransformer>().FirstOrDefault( x => x.logicalClass.Equals(associationEnd.type));
			}
			if (transformer == null)
			{
		...
		protected MySQLTableTransformer addMySQLTable(Class classElement)
		{
			MySQLTableTransformer transformer = null;
			if (classElement.owningPackage.Equals(this.logicalPackage))
			{
				...
				{
					transformer = new MySQLTableTransformer(this._newDatabase,_nameTranslator);
			...
			else
			{
				...
				{
					transformer = new MySQLTableTransformer(this._externalDatabase,_nameTranslator);
			...
			if (transformer != null)
			{
				...
				foreach (var dependingAssociationEnd in transformer.getDependingAssociationEnds()) 
				{
					...
					{
						var dependingEndTransformer = addMySQLTable(dependingAssociationEnd);
		...
		protected override void nameUnnamedTables()
		{
			...
			{
				foreach (MySQLForeignKeyTransformer foreignKeyTransformer in foreignKeyTransformers) 
				{
					...
					foreach (var foreignKeyTransformer in foreignTabletransformers.foreignKeyTransformers.Where(x => x.foreignKey.foreignTable == abstractTableTransformer.table)) 
					{
						foreach (var subclasses in abstractTableTransformer.logicalClasses.Select(x => x.subClasses))
						{
							foreach (var subclass in subclasses) 
							{
								...
								var newForeignKeyTransfomer = new MySQLForeignKeyTransformer((Table)foreignKeyTransformer.foreignKey.ownerTable ,correspondingsubColumns,(MySQLTableTransformer)subTabletransformer,(AssociationEnd)foreignKeyTransformer.logicalAssociationEnd,this._nameTranslator);
							}
						}
