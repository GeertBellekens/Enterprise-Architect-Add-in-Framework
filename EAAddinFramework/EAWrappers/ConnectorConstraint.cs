...
namespace TSF.UmlToolingFramework.Wrappers.EA
{
    ...
    /// </summary>
    public class ConnectorConstraint : Element, UML.Classes.Kernel.Constraint
    {
        internal EADBConnectorConstraint wrappedConstraint { get; set; }
        // private Element _owner;
        private OpaqueExpression _opaqueExpression;
        private ConnectorWrapper connectorWrapper => this.owner as ConnectorWrapper;
        //private ElementWrapper elementWrapperOwner => this.owner as ElementWrapper;
        public ConnectorConstraint(Model model, EADBConnectorConstraint wrappedConstraint) : base(model)
        {
        ...
        public override UML.Classes.Kernel.Element owner
        {
            get
            {
                if (this._owner == null)
                {
                    this._owner = this.EAModel.getRelationByID(this.wrappedConstraint.ConnectorID);
                }
                return this._owner;
            }
        ...
        {
            for (int i = this.connectorWrapper.wrappedConnector.Constraints.Count - 1; i >= 0; i--)
            {
                var currentConstraint = (global::EA.ConnectorConstraint)this.connectorWrapper.wrappedConnector.Constraints.GetAt(short.Parse(i.ToString()));
                if (this.name == currentConstraint.Name)
                {
                    this.connectorWrapper.wrappedConnector.Constraints.DeleteAt(short.Parse(i.ToString()), false);
                }
            }
        }
        ...
        {
            get => this.connectorWrapper.owningPackage;
            set => throw new NotImplementedException();
        }
        ...
        #endregion
        public int connectorID => this.wrappedConstraint.ConnectorID;
        public static void loadConstraints(Dictionary<int, ConnectorWrapper> connectorDictionary, Model model)
        {
            var eaDBConnectorConstraints =
                    EADBConnectorConstraint.getEADBConnectorConstraintsForConnectorIDs(connectorDictionary.Keys, model);
            var connectorConstraints = model.factory.createElements(eaDBConnectorConstraints).OfType<ConnectorConstraint>();
            //add the connectors to their respective elements
            foreach (var connectorConstraint in connectorConstraints)
            {
                if (connectorDictionary.TryGetValue(connectorConstraint.connectorID, out ConnectorWrapper connectorWrapper))
                {
                    connectorWrapper.addExistingConstraint(connectorConstraint);
                }
            }
        }
    }
